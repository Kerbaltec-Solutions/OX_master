...
public class ox_master extends PApplet {
    ...
    while(line!=null){
      try{
        ...
        if(line!=null){
          ...
          box[PApplet.parseInt(line[1])]=PApplet.parseInt(split(line[0],';'));
          println(line[1]);
        }
      }catch (IOException e){}
    }
...
public void draw(){
  if(setup){
    ...
    text(PApplet.parseInt(index/(PApplet.parseFloat(millis())/1000))+" index/sec",width/2,height/2+100);
    index++;
    if(index==19682){
      textAlign(LEFT);
      sig.stop();
...
public void kreis(){
  ...
  verl = split(verlauf,';');
  for(;runde>=0;runde--){
    data=PApplet.parseInt(split(verl[runde],','));
    prop=box[data[0]];
    if(prop[data[1]]>0){
...
public int check(int tocheck){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ox_master" };
    if (passedArgs != null) {
